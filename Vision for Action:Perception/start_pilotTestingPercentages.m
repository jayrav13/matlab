...
fprintf(fid, '  2 == reverspective\n');
fprintf(fid, '  3 == reverslective\n');
    ...
    while(sum(success) ~= 36)
        for m=11:46
            ...
            if(success(current_m) == 0)
                ...
                fprintf(fid, '%d\t\t%d\t\t%d/%d/%d, %d:%d:%d\t%d\t\t%d\t\t%d\t\t%s\t\t%d''\n',m,current_m,temp_time(2),temp_time(3),temp_time(1),temp_time(4),temp_time(5),temp_time(6),current_scramble(1),current_scramble(2),current_scramble(3),status,further_spot(m));
            end
        end
        done_one = 1;
        if(count == 1)
            fprintf(fid, 'main trials -- failed redo\n');
            fprintf(fid, 'ov_trial_#\tma_trial_#\ttime\t\t\t\tstimuli\tmotion\tspeed\t\tresult\ttarget\n');
